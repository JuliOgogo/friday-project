    ...
    switch (action.type) {
        case "packs/ADD_CARDS":
            return {
                ...state,
                cardPacks: action
                    .cardPacks.map(({_id, name, user_name, updated, cardsCount}) => ({
                        _id, name, cardsCount, updated: new Date(updated).toLocaleDateString(), user_name
                    }))
            }
        case "packs/ADD_CARD":
            return {
                ...action.cardPack,
                cardPacks: action.cardPack.cardPacks.map(({_id, name, user_name, updated, cardsCount}) => ({
                    _id, name, cardsCount, updated: new Date(updated).toLocaleDateString(), user_name
                }))
            }
